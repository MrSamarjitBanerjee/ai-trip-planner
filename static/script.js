...
async function sendMessage() {
  ...
  if (waitingForApproval) {
    showError("There is already a draft waiting for your review. Approve it or request changes first.");
    return;
  }
...
async function submitApproval(approved) {
  ...
  try {
    ...
    // A revision re-runs the supervisor and pauses again at a new draft,
    // so keep the approval panel visible and refresh the draft. Only when
    // the user approves does the flow finish.
    if (data.requires_approval) {
      showResult(data.itinerary || data.answer, data.thread_id, true);
      showApproval(data);
    } else {
