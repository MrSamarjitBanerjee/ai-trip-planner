...
def _subprocess_env(**updates: str | None) -> dict[str, str]:
    """Current environment plus any extras, handed to MCP subprocesses."""
...
client = MultiServerMCPClient(
    {
        # tavily runs remotely over http; the api key rides along in the url
        "tavily": {
        ...
        "aviationstack": {
            "transport": "stdio",
            "command": UVX_COMMAND,
            # pinning the mcp sdk version here - newer releases stopped
            # shipping mcp.server.fastmcp, which this server still imports
            "args": [
                "--with",
                "mcp==1.28.1",
                "aviationstack-mcp",
        ...
        # our own weather server, launched with this same interpreter
        "weather": {
            "transport": "stdio",
...
async def _get_server_tool(
    ...
    """Loading a single tool from a single MCP server."""
    ...
    # connecting to only this server, so a broken weather or aviation server
    # can't take down an unrelated tavily call
    # loading tools from just this server, so one dead server can't
...
async def get_all_tools() -> None:
    """Pinging each MCP server once and reporting what answers."""
