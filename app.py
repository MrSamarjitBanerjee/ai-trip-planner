...
# serving the boarding-pass style UI
...
async def travel_planner(request_data: TravelRequest):
    try:
        ...
        # pushing the graph into a worker thread; a full run can sit on
        # LLM calls for a couple of minutes
...
async def approve_travel_plan(request_data: ApprovalRequest):
    try:
        ...
        # waking up a paused thread with the reviewer's decision
        result = await run_in_threadpool(
            resume_travel_agent,
            thread_id=request_data.thread_id,
...
# liveness probe, also what render pings
@app.get("/health")
